...
struct node {
  string title;              // data: the value of this node
  string type;               // type: type of media
  double release;            // stores release date as a double for example Jan 1 2023 would be 20230101
  shared_ptr<node> next;  // next: pointer to the next node in the list, or NULL if this is the last node.
  shared_ptr<node> prev;  // prev: previous node in chronological or release order, depending on user preference
